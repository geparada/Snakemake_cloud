...
txi <- tximport(input_files, type="salmon", txOut=TRUE)
coldata <- data.frame(
...
res_table$transcript_ID <- rownames(as.data.frame(res))


fwrite(res_table[ , .(transcript_ID, baseMean, log2FoldChange, lfcSE, stat, pvalue, padj)], output_file, sep='\t')
